...
All animations live in `design-system/motion/animations.css`. Apply them by
...
Open `deck-templates/showcases/animation-showcase.html` to browse all of them — one slide
...
when it leaves. They are loaded by `design-system/motion/fx-runtime.js`, which
dynamically pulls every module under `design-system/motion/fx/*.js` and watches
`.slide.is-active` to run lifecycle.

Add to any page:
```html
<script src="../design-system/motion/fx-runtime.js"></script>
```
