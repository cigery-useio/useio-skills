...
(function () {
  ...
  ready(function () {
    ...
    const CHANNEL_NAME = 'html-ppt-creator-presenter-' + location.pathname;
    let bc;
    ...
    /* ===== nav buttons (clickable prev/next) ===== */
    let navButtons = document.querySelector('.deck-nav');
    if (!navButtons) {
      navButtons = document.createElement('div');
      navButtons.className = 'deck-nav';
      navButtons.innerHTML = '<button class="deck-nav-btn deck-nav-prev" aria-label="上一页"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button><span class="deck-nav-page"><span class="cur">1</span> / '+total+'</span><button class="deck-nav-btn deck-nav-next" aria-label="下一页"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></button>';
      document.body.appendChild(navButtons);
      navButtons.querySelector('.deck-nav-prev').addEventListener('click', function(e){ e.stopPropagation(); go(idx-1); });
      navButtons.querySelector('.deck-nav-next').addEventListener('click', function(e){ e.stopPropagation(); go(idx+1); });
    }
    const navPrev = navButtons.querySelector('.deck-nav-prev');
    const navNext = navButtons.querySelector('.deck-nav-next');
    const navPageCur = navButtons.querySelector('.deck-nav-page .cur');
    ...
    function go(n, fromRemote){
      ...
      if (navPrev) navPrev.disabled = (n <= 0);
      if (navNext) navNext.disabled = (n >= total - 1);
      if (navPageCur) navPageCur.textContent = String(n + 1);
    ...
    function buildPresenterHTML(deckUrl, slideMeta, total, startIdx, channelName, currentTheme) {
      ...
      const storageKey = 'html-ppt-creator-presenter:' + location.pathname;
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    if (!themeBase) {
      ...
      if (existingLink) {
        ...
        themeBase = lastSlash >= 0 ? rawHref.substring(0, lastSlash + 1) : 'design-system/themes/';
      } else {
        themeBase = 'design-system/themes/';
      }
    }
